    ...
    class RequestHandler(BaseHTTPRequestHandler):
        ...
        def _dispatch(self, request: dict[str, Any]) -> dict[str, Any]:
            ...
            if self.path == "/evaluate-rebooking":
                ...
                incident_sequence = plan.get("incident_sequence", 1)
                if (
                    isinstance(incident_sequence, bool)
                    or not isinstance(incident_sequence, int)
                    or incident_sequence < 1
                ):
                    raise ToolError(
                        "INVALID_INCIDENT_SEQUENCE",
                        "incident_sequence must be a positive integer",
                    )
                case["incident_sequence"] = incident_sequence
                case["case_state"] = "RESOLVING"
