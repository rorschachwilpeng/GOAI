...
4. Publish an `ORDER_LINKED` handoff to the Case Project Room only after a
   ...
   during a direct runtime handoff. A trusted `[GOAI PROJECT HANDOFF]` display
   request instead supplies the complete event and must be published without a
   peer mention, keeping internal execution outside the Project display. Do not
   use `copaw channels list`, guess a Room, or read credentials.
