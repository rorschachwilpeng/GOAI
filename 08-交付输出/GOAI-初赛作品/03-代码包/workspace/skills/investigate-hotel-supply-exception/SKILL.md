...
For this MVP, the Resolution Agent's dedicated Worker Room is also the
Operations Review Room shown to Hotel Operations. A `P2_REVIEW_START` without
a Matrix mention is a human review notice and requires no Agent reply; the
controller validates the operator decision and separately requests the final
Project Room summary.

Only in that dedicated Operations Review Room, an admin-mention
