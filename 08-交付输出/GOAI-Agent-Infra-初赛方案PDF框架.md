...
多 Room 协作，其实是 Agent 身份边界在业务场景中的延伸。客服与售后需要频繁配合，因此被放进同一个“协作会议室”，共享当前问题所需的部分上下文；主控 Agent 则与每个子 Agent 保留独立聊天 Room，会议室里只谈当前任务，不是什么都聊，这其实和人类的真实协同是一致的。主控按需调度，并只同步必要信息。评估 Agent 也被单独放在一个 Room, 让它站在“旁观者”视角，不受冗余上下文干扰，给出中立判断。这样的设计兼顾了上下协调、同级协作和客观评估。
