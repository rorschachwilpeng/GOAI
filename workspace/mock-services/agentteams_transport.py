...
class MatrixAdminClient:
    ...
    def _send_manager_copaw(
        ...
        event = self.wait_for_message(
            ...
            after_ms=started_ms,
            predicate=lambda value: value == body,
            timeout_seconds=15,
        )
...
class DockerAgentTeamsTransport:
    ...
    def request_frontline_project_handoff(
        ...
        *,
        ...
        self.matrix.send_admin(self.frontline_room_id, prompt)
        return self.wait_project_event(
    ...
    def request_resolution_project_update(
        ...
        *,
        ...
        started_ms = int(time.time() * 1000) - 1000
        body = (
            f"{self.frontline_matrix_id}\n"
            f"{json.dumps(event, ensure_ascii=False, separators=(',', ':'))}"
        )
        prompt = (
            f"{self.resolution_matrix_id} [GOAI PROJECT UPDATE]\n"
            f"A valid Project Room handoff exists at {source_event_id}. "
            "Use your assigned Skill to send exactly the two-line message below to the "
            "already-assigned Case Project Room. Do not add commentary or alter the JSON.\n"
            f"{body}"
        )
        self.matrix.send_admin(self.resolution_room_id, prompt)
        return self.wait_project_event(
    ...
    def request_operations_review(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        started_ms = int(time.time() * 1000) - 1000
        prompt = (
            f"{self.resolution_matrix_id} [GOAI OPERATIONS REVIEW REQUEST]\n"
            "Present this synthetic high-risk review request to Hotel Operations. Ask for "
            "exactly APPROVE or REJECT; do not execute the plan.\n"
            f"{json.dumps(request, ensure_ascii=False, separators=(',', ':'))}"
        )
        self.matrix.send_admin(self.resolution_room_id, prompt)
        event = self.matrix.wait_for_message(
            room_id=self.resolution_room_id,
            sender=self.resolution_matrix_id,
            after_ms=started_ms,
            predicate=lambda body: str(request.get("case_id")) in body,
            timeout_seconds=90,
        )
        return {"matrix_event_id": event["event_id"], "after_ms": started_ms}
    ...
    def publish_project_event(self, event: dict[str, Any]) -> str:
        ...
        self._ensure_ready(str(event["receiver"]).lower())
    ...
    def bind_resolution_task(
        ...
        *,
        ...
        event = {
            ...
            "receiver": "RESOLUTION",
            "conclusion": "Resolution is assigned to the referenced Frontline handoff.",
            "next_action": "Read the referenced event and publish one strict JSON Resolution status event.",
            "evidence_ref": f"matrix-event://{frontline_event_id}",
    ...
    def route_operations(self, decision: dict[str, Any]) -> str:
        self._ensure_ready("resolution")
        prompt = (
            f"{self.resolution_matrix_id} [GOAI OPERATIONS DECISION]\n"
