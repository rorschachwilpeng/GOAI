...
class RehearsalController:
    ...
    def _handle_customer_message(self, message: dict[str, Any]) -> None:
        ...
        if self.stage == self.STAGE_FIRST_CONFIRMATION:
            ...
            second_handoff = _project_event(
                ...
                sender="FRONTLINE",
                receiver="RESOLUTION",
                conclusion="The customer-owned order remains linked for incident 2.",
                next_action="Investigate the recurring supplier exception.",
                evidence_ref=f"order-ref://{plan['order_ref']}",
            )
            second_frontline_event = self.transport.request_frontline_project_handoff(
                second_handoff
            )
            proposal = _project_event(
                event_type="RESOLUTION_PROPOSED",
            ...
            resolution_event = self.transport.request_resolution_project_update(
                proposal,
                source_event_id=second_frontline_event["matrix_event_id"],
            )
    ...
    def _handle_operations_decision(
        ...
        summary_event = self.transport.request_resolution_operations_summary(
            summary,
